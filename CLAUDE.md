...
**Multi-function menu + typed staging (v0.9.13.0+)** — the bookmarklet is now menu-driven: tapping it shows **① Capture new saves** (the flow above) and **② Refresh scores**. Refresh pages the **entire** active library from Supabase (`deleted_at=is.null&order=saved_at.desc`, via `Range`/`Range-Unit` headers, 1000/page), batches them through Reddit's same-origin `/api/info.json?id=…` (100 ids/call, 1s pacing), and **flushes `{op:'score',reddit_id,score}` rows to the inbox per batch** so an interrupted run still stages its progress (`sent` counter reported in banners; 429 retries the same batch).
...
`key, value, updated_at` — stores: `redditFeedUrl`, `redditUsername`, `feedProxyUrl`, `feedProxyType`, `confirmDestructive`, `last_modified`
